                            ...
                            <div class="col-12">
                                <div class="todo-panel card border-0 shadow-sm">
                                    <div class="todo-panel__head">
                                        ...
                                        <span class="todo-panel__count todo-panel__count--done"
                                            id="finishedCountBadge">0</span>
                                    </div>
                                    <div class="card-body p-0">
                ...
                <div class="modal-body pt-3">
                    <form id="todoForm" class="row g-3">
                        <div class="col-md-12">
                            ...
                            <input type="text" id="todoTitle" class="form-control" maxlength="255" placeholder="Enter Title" required>
                        </div>
                        
                        <div class="col-12">
                            <label for="todoDescription" class="form-label fw-semibold">Description</label>
                            <textarea id="todoDescription" class="form-control todo-description-area" rows="4"
                                placeholder="Add extra details for this task"></textarea>
                        </div>
                        ...
                        <div class="col-12">
                            ...
                            <div class="form-text">You can upload multiple files. When editing a todo, new files are added
                                to the existing attachments.</div>
                        ...
                        {{-- Reminder Settings --}}
                        <div class="col-12">
                            <div class="todo-panel__eyebrow">Task Reminder</div>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="input-group">
                                <input type="number" id="todoRepeatInterval" class="form-control" min="1"
                                    value="1" required>
    ...
    @push('styles')
        <style>
            ...
            .todo-hero {
                ...
                gap: 24px;
                padding: 28px;
                background: linear-gradient(349deg, var(--todo-brand-navy) 0%, var(--todo-brand-cyan) 58%, #7dd7ee 100%);
                color: #fff;
                overflow: hidden;
                position: relative;
            }
            ...
            .todo-new-btn {
                ...
                font-weight: 700;
                background: #ffffff;
                color: var(--todo-brand-navy);
                border: 0;
                box-shadow: 0 12px 30px rgba(9, 24, 65, 0.18);
            }

            .todo-new-btn:hover {
                color: var(--todo-brand-indigo);
                background: #f8fcff;
            }
            ...
            .bg-soft-blue {
                background: #e9f9fd;
                color: var(--todo-brand-cyan);
            }

            .bg-soft-gold {
                background: #fff4e8;
                color: var(--todo-brand-orange);
            }
            ...
            .todo-note-box {
                ...
                padding: 18px;
                border-radius: 18px;
                background: linear-gradient(135deg, var(--todo-brand-cyan), #78d8f0);
                color: #ffffff;
            }

            .todo-note-box__title {
                font-weight: 700;
                margin-bottom: 8px;
            }
            ...
            .todo-item {
                ...
                background: #ffffff;
                border: 1px solid var(--todo-border);
                box-shadow: 0 10px 24px rgba(96, 126, 173, 0.08);
            }
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                async function handleListAction(event) {
                    ...
                    const todo = todos.find(function(item) {
                        return String(item.id) === String(todoId);
                    });
                    if (!todo) return;
                ...
                function collectPayload() {
                    ...
                    const repeatDays = Array.from(document.querySelectorAll(
                        '#repeatDaysWrap input[type="checkbox"]:checked')).map(function(item) {
                        return item.value;
                    });
                    ...
                    return {
                        ...
                        ends_after_occurrences: endsType === 'after' ? Number(fields.ends_after_occurrences.value ||
                            0) || null : null,
                ...
                function render() {
                    const unfinished = todos.filter(function(item) {
                    ...
                    const dueToday = unfinished.filter(function(item) {
                        return normalizeDate(item.task_date) === todayIso;
                    });
                ...
                function todoTemplate(todo) {
                    ...
                    const attachmentHtml = attachments.length ?
                        '<div class="todo-attachment-inline">' + attachments.map(function(attachment) {
                            ...
                            return '<a class="todo-attachment-chip" href="' + escapeHtml(attachment.url || '#') +
                                '" target="_blank" rel="noopener"><i class="bx bx-paperclip"></i>' + escapeHtml(
                                    fileName) + '</a>';
                        }).join('') + '</div>' :
                        '';
                    ...
                    return [
                        ...
                        '<input class="form-check-input todo-check todo-toggle" type="checkbox" ' + (todo
                            .is_completed ? 'checked' : '') + '>',
                        '<div class="flex-grow-1">',
                        '<div class="' + titleClass + '">' + escapeHtml(todo.title || '') + '</div>',
                        '<div class="' + descClass + '">' + escapeHtml(todo.description || 'No description') +
                        '</div>',
                        '<div class="todo-meta">',
                        '<span class="todo-badge"><i class="bx bx-calendar"></i> ' + escapeHtml(formatDate(todo
                            .task_date)) + '</span>',
                        '<span class="todo-badge"><i class="bx bx-time"></i> ' + escapeHtml(formatTime(todo
                            .task_time) || 'Any time') + '</span>',
                        '<span class="todo-badge"><i class="bx bx-refresh"></i> ' + escapeHtml(todo
                            .display_schedule || '') + '</span>',
                        '<span class="todo-badge"><i class="bx bx-play-circle"></i> ' + escapeHtml('Starts ' +
                            formatDate(todo.starts_on)) + '</span>',
                        '<span class="todo-badge"><i class="bx bx-stop-circle"></i> ' + escapeHtml(endsText) +
                        '</span>',
                        (repeatDays ? '<span class="todo-badge"><i class="bx bx-repeat"></i> ' + escapeHtml(
                            repeatDays) + '</span>' : ''),
                ...
                function renderExistingAttachments(attachments) {
                    ...
                    todoExistingAttachments.innerHTML = items.map(function(attachment, index) {
                        const fileName = attachment.name || attachment.path || 'Attachment';
                        const removeBtn = '<span class="todo-attachment-remove" data-index="' + index +
                            '" title="Remove"><i class="bx bx-x"></i></span>';
                        return '<a class="todo-attachment-chip" href="' + escapeHtml(attachment.url || '#') +
                            '" target="_blank" rel="noopener"><i class="bx bx-paperclip"></i>' + escapeHtml(
                                fileName) + removeBtn + '</a>';
                    }).join('');
