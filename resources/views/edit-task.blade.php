                ...
                <form action="{{ route('edit-task.update', $task->id) }}" method="POST" class="row g-3" enctype="multipart/form-data">
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="project_related" name="project_related" class="form-select">
                            ...
                            @foreach($projects as $project)
                                <option value="{{ $project->id }}" {{ old('project_related', $task->project_id) == $project->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <input
                            ...
                            class="form-control"
                            id="due_date"
                            value="{{ old('due_date', $task->deadline ? $task->deadline->format('Y-m-d') : '') }}"
                        >
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="assignees" name="assignees[]" class="form-select select2" multiple>
                            @foreach($staff as $member)
                                <option value="{{ $member->id }}" {{ in_array($member->id, old('assignees', $task->assignees ?? [])) ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="followers" name="followers[]" class="form-select select2" multiple>
                            @foreach($staff as $member)
                                <option value="{{ $member->id }}" {{ in_array($member->id, old('followers', $task->followers ?? [])) ? 'selected' : '' }}>
                                    {{ $member->first_name }} {{ $member->last_name }}
                                </option>
                            @endforeach
                        </select>
